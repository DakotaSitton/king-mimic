...
export const KIT = {
  ...
  oRainblow:   { name: "Rainblow Blade", ante: 1, cost: 3, icon: "🌈", color: "#c07fe8", text: "In 6 seconds: strike your whole foe lane for your melee + ranged bonuses.",
                 ops: [{ do: "timer", period: 60, once: true, ops: [{ do: "deal", amount: 0, target: "lane", bothKinds: true }] }] }, // FLAGGED: cost 3 picked; base 0 = pure "melee + ranged" scaling per the owner's words. The delayed lane strike fires BOTH melee AND ranged play-triggers when it resolves (owner 2026-07-09, mirrors Moonlight's lane form). FLAG: Rainblow ALSO classifies "ranged" at CAST time (its inner lane deal touches foes), so onPlayRanged fires once at cast AND again at the strike, while onPlayMelee fires only at the strike — left AS-IS; say if the cast-time ranged trigger should be suppressed so one Rainblow = one balanced both-trigger.
