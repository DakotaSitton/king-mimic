...
export const KIT = {
  ...
  oDemonForm:  { name: "Demon Form",   ante: 1, cost: 3, lasting: true, icon: "😈", color: "#b85c6e", text: "This fight: pick melee or ranged — gain +1 to that kind every 6 seconds, and deal 1 to the front foe every 6 seconds.",
                 ops: [{ do: "regen", kind: "modalBonus", amount: 1, period: 60 },
                        { do: "timer", period: 60, ops: [{ do: "deal", amount: 1, target: "front" }] }] }, // FLAG: cost 2 picked pre-R2 (now 3). +1/6s modal bonus = owner's numbers. DAMAGE TICK (owner 2026-07-10 "deal 1 damage every 6 seconds"): reuses the Animated-Blade timer→deal pattern. FLAG target="front" (front foe in the owner's lane — matches Animated Blade's every-6s tick; owner confirm vs aimed "pick"). FLAG 1 dmg / 6s (period 60). TYPE NOTE: the foe-damage tick makes Demon Form derive foe-affecting (opsTouchFoes recurses into the timer) → triggerKind "none"→"ranged" & isRanged→true (🎯 badge, fires ranged play-triggers at cast, takes Lizard-Wizard ranged pricing). cardKind stays "untyped" (no TOP-LEVEL deal) so archetype-fit / foeCombatStat / itemFlavor are UNCHANGED. Owner confirm if he wants it to stay typeless.
  ...
  oSageMode:   { name: "Sage Mode",    ante: 1, cost: 5, lasting: true, icon: "🧙", color: "#8a9cff", text: "This fight: heal 2 every 6 seconds.", ops: [{ do: "regen", kind: "heal", amount: 2, period: 60 }] }, // FLAG: heal 2 (owner "heals 2" — already 2, unchanged). COST net +2 vs pre-R2: R2's global +1 → 4, plus owner's explicit "+1 more moxie" (2026-07-10) → 5. Owner confirm if he wanted +1 TOTAL (cost 4) instead of +1 on top of R2. NOTE: heal-2 duplicates Trollskin Tiara — owner may want to differentiate.
  ...
  oTeleBlades: { name: "Telekinetic Blades", ante: 1, cost: 4, lasting: true, icon: "🔮", color: "#9b8cff", text: "This fight: your melee cards strike your AIMED foe instead of the front, scaling with your ranged bonus.", ops: [{ do: "tkBlades" }] }, // FLAGGED: cost 3 picked; read = melee cards aim (front→pick) + take the RANGED bonus; play-triggers still melee — say if those should flip too (R2 +1 → cost 4)
  oGiantsBelt: { name: "Giant's Belt", ante: 1, cost: 6, lasting: true, icon: "🥋", color: "#a0b070", text: "This fight: your max health grows by your base health (once) and you heal the gained amount.", ops: [{ do: "giantBelt" }] }, // FLAGGED: cost 5 picked; duration read as THIS FIGHT (owner said "passive" — worn passives are dead per his own 7/06 ruling, so it's a lasting cast like Stoneskin). NERF (owner 2026-07-10): +base health ONCE, non-compounding (was framed as a "double"); magnitude FLAGGED in applyGiantBelt. (R2 +1 → cost 6)
  ...
  oCrystalBall:{ name: "Crystal Ball", ante: 1, cost: 4, ranged: true, icon: "🧿", color: "#b48fe0", text: "Put a card of your choice from your deck into your hand; gain +1 ranged damage this fight.",
